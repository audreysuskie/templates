...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError('The account assigned to this token no longer exists.', 401)
    );
  }
...
exports.restrictTo =
  ...
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(
          `You do not have permission to perform this action. You must be an administrator to perform this action.`,
          403
        )
      );
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('An account with this email address could not be found', 404)
    );
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError(
        'Your current password is incorrect. Request unauthorized.',
        401
      )
    );
  }
